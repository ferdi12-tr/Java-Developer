...
public class Main {
    ...
    public static String[][] fogGameField = new String[11][11]; //to fog the battle-ship, we show the shots on that
    ...
    private static void takeAShoot() { // we can shoot the war-ship thanks to this method
        ...
        while (true) {
            ...
            try {
                ...
                if (Objects.equals(gameField[row][column], "O ") || Objects.equals(gameField[row][column], "X ")) {
                    gameField[row][column] = "X "; // fill the gameField
                    fogGameField[row][column] = "X "; // fill the fogGameField

                    printField(fogGameField); // first print fogGameField

                    if (isSunkShip(row, column)) { // pass the current hit coordinate
                    ...
                    gameField[row][column] = "M "; // fill the gameField with "M " because we missed
    ...
    private static boolean isLastShip() {
        boolean notLastShip = false;
        for (int i = 1; i < 11; i++) {
            for (int j = 1; j < 11; j++) {

                if (Objects.equals(gameField[i][j], "O ")) { // make sure we do not contain any ship which is not hit
                    notLastShip = true;
                    break;
                }
    ...
    private static boolean isSunkShip(int row, int column) {
        ...
        int i = row - 1;
        while (i >= 0 && !gameField[i][column].equals("M ") && !gameField[i][column].equals("~ ")) {
            if (gameField[i][column].equals("O ")) {
                checkRowFront = true;
                break;
            }
            i--;
        ...
        int j = row + 1;
        while (j <= 10 && !gameField[j][column].equals("M ") && !gameField[j][column].equals("~ ")) {
            if (gameField[j][column].equals("O ")) {
                checkRowBack = true;
                break;
            }
            j++;
        ...
        int k = column - 1;
        while (k >= 0 && !gameField[row][k].equals("M ") && !gameField[row][k].equals("~ ")) {
            if (gameField[row][k].equals("O ")) {
        ...
        int m = column + 1;
        while (m <= 10 && !gameField[row][m].equals("M ") && !gameField[row][m].equals("~ ")) {
            if (gameField[row][m].equals("O ")) {
    ...
    public static void initializeField(String[][] field) { // initialize the taken fields
